...
/*
{
  ...
  "content_scripts": [
    {
      "matches": ["https://www.sinotrade.com.tw/*"],
      "js": ["content.js"]
    }
  ]
}
...
window.autoOrder = async (codes) => {
  for (const code of codes) {
    const input = await waitForElement('#app-container input');
    input.value = code;
    input.dispatchEvent(new Event('input', { bubbles: true }));

    const item = await waitForElement('#ui-id-2 li');
    item.click();

    const btn = await waitForElement('button.midbtn.submit');
    btn.click();
  }
};
